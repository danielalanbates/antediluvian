        ...
        (
            ...
            (
                ...
                buffer_layout(
                    ...
                    false,
                    // LOCAL FIX (not in upstream 0.15.3) — breaks WebGL2.
                    //
                    // Without storage buffers, mesh_view_bindings.wgsl declares
                    // this binding as `array<vec4<f32>, 64>`, a fixed 1024-byte
                    // uniform. Upstream declares a 16-byte minimum here
                    // regardless, so pipeline creation fails validation with
                    // "Shader global ResourceBinding { group: 0, binding: 14 }
                    // is not available in the pipeline layout" and every PBR
                    // draw panics — i.e. the browser build cannot render at all.
                    //
                    // bevy_render already pads the buffer itself to 64 elements
                    // in the uniform case, so only this declaration was wrong.
                    Some(match visibility_ranges_buffer_binding_type {
                        BufferBindingType::Storage { .. } => Vec4::min_size(),
                        // 64 elements * 16 bytes; the count is
                        // bevy_render's private VISIBILITY_RANGE_UNIFORM_BUFFER_SIZE.
                        BufferBindingType::Uniform => {
                            NonZero::<u64>::new(64 * 16).expect("non-zero")
                        }
                    }),
                )
